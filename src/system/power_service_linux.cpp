...
namespace desktop::system
{
	...
	bool power_service::allow_suspend()
	{
		...
		GDBusProxy* proxy{ g_dbus_proxy_new_for_bus_sync(G_BUS_TYPE_SESSION, G_DBUS_PROXY_FLAGS_NONE, nullptr, "org.freedesktop.ScreenSaver",
	...
	bool power_service::prevent_suspend()
	{
		...
		GVariant* result{ g_dbus_proxy_call_sync(proxy, "Inhibit", g_variant_new("(ss)", "desktop", "Preventing suspend"), G_DBUS_CALL_FLAGS_NONE, -1, nullptr,
			                                     &error) };
		if (error)
		{
			g_error_free(error);
