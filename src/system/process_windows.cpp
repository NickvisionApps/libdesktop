...
namespace desktop::system
{
    ...
    bool process::impl::start()
    {
        ...
        if(!m_process.m_working_directory.empty() && (!std::filesystem::exists(m_process.m_working_directory) || !std::filesystem::is_directory(m_process.m_working_directory)))
        {
            return false;
        }
        try
        {
            ...
            if(!CreatePipe(&m_stdout_read, &m_stdout_write, &security_attributes, 0) ||
                !SetHandleInformation(m_stdout_read, HANDLE_FLAG_INHERIT, 0) ||
                !CreatePipe(&m_stderr_read, &m_stderr_write, &security_attributes, 0) ||
                !SetHandleInformation(m_stderr_read, HANDLE_FLAG_INHERIT, 0) ||
                !CreatePipe(&m_stdin_read, &m_stdin_write, &security_attributes, 0) ||
            ...
            if(!CreateProcessW(nullptr,
                ...
                nullptr,
                working_directory_str.empty() ? nullptr : working_directory_str.c_str(),
                &startup_info,
                &m_process_information))
            {
                cleanup();
